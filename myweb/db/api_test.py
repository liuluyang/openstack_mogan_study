...
class connection(object):
    ...
    def user_update(self, values, user_id):
        session = get_session()
        user_query = session.query(models.User).filter_by(id=user_id)#.options(joinedload('articles'))

        try:
            user_ref = user_query.one()
            ref = update(user_ref, values)
            session.add(ref)
            session.commit()
...
new_user = db_server.user_update({'username':'joke'}, 1)
new = dict(new_user)
print new
print new_user.articles
print new_user['articles']
